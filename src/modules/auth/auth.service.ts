...
export class AuthService {
    ...
    async rotateRefreshToken(combinedToken: string) {
        ...
        if (splitToken.length !== 2) throw new UnauthorizedException('Token mal formado');
        ...
        if (!tokenDb) throw new UnauthorizedException('Token inválido o no encontrado');
        ...
        if (tokenDb.is_revoked) {
            ...
            await this.prismaService.refreshToken.deleteMany({ where: { user_id: userId } });
            throw new UnauthorizedException('Token revocado. Sesión cerrada por seguridad.');
        }
